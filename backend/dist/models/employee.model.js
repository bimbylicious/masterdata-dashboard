...
export class EmployeeModel {
    ...
    async bulkCreate(employees) {
        ...
        for (const emp of employees) {
            try {
                const existing = await pool.query('SELECT empcode FROM employees WHERE empcode = $1', [emp.empcode]);
    ...
    async updateOrCreate(employees) {
        const updated = [];
        const inserted = [];
        let skipped = 0;
        for (const emp of employees) {
